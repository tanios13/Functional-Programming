...
object RecursiveLanguage {
    ...
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => 
        eval(scrutinee, defs) match
          ...
          case Cons(h, t) => 
            eval(caseCons, defs ++ Map(headName -> h, tailName -> t))
          case _ => error("Evaluating a match with an invalid scrutinee did not raise an error.")
    ...
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) =>
        if headName == n || tailName == n then
          ...
          val fvs = freeVars(r)
          if(fvs.contains(tailName) || fvs.contains(headName)) then
            val newTailName = differentName(tailName, fvs)
            val newHeadName = differentName(headName, fvs)
          // Perform alpha conversion in caseCons to eliminate the name collision.
            Match(subst(scrutinee,n,r), subst(caseEmpty,n,r), newHeadName, tailName,
              subst(alphaConvert(alphaConvert(caseCons, tailName, newTailName),  headName, newHeadName),n,r))
  ...
  def freeVars(e: Expr): Set[String] =
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => freeVars(scrutinee) ++ freeVars(caseEmpty)
  ...
  def show(e: Expr): String =
    e match
      ...
      case Match(scrutinee, caseEmpty, headName, tailName, caseCons) => 
          s"${show(scrutinee)} match case Nil => ${show(caseEmpty)} case ${(headName)} :: ${(tailName)} => ${show(caseCons)} "                                                                               
  ...
  val definitions: DefEnv = Map[String, Expr](
    ...
    "foldLeft" -> Fun("ls", Fun("acc", Fun("f",
                    ...
                    "x", "xs", Call(
                                Call(
                                    Call(N("foldLeft"),
                                        N("xs")), Call(Call(N("f"), N("acc")), N("x"))), N("f")
                                            ))))
                          ),
    ...
    //  (ls => (z => (fold => ls match 
    //       { case nil => z; 
    //         case x :: xs => fold(x)(foldRight(xs)(z)(fold)) })))
    ...
    "foldRight" -> Fun("ls", Fun("z", Fun("fold",
                    ...
                    "x", "xs", Call(
                                    Call(N("fold"),
                                        N("x")), Call(Call(Call(N("foldRight"), N("xs")), N("z")), N("fold"))
                                            ))))
                          ),
  )
