...
trait Solver extends GameDef:
  ...
  def neighborsWithHistory(b: Block, history: List[Move]): LazyList[(Block, List[Move])] = 
    def neighborsWithHistoryRec(legalNeighbors: List[(Block, Move)]): List[(Block, List[Move])] = legalNeighbors match
      case Nil => Nil
      case (block, move) :: xs => (block, move :: history) :: neighborsWithHistoryRec(xs)
    neighborsWithHistoryRec(b.legalNeighbors).to(LazyList)
  ...
  def from(initial: LazyList[(Block, List[Move])],
           explored: Set[Block]): LazyList[(Block, List[Move])] = 
    if(initial.isEmpty) then LazyList()
    else  
      val solutions =
        for 
          (block, moves) <- initial
        ...
        yield next
      initial #::: from(solutions, explored ++ solutions.map((x,y) => x))
