...
trait DecoderInstances:
  ...
  given Decoder[Int] =
    Decoder.fromFunction { case Json.Num(n) => if(n.isValidInt) then Option(n.intValue) else None
                           case _ => None}
  ...
  given [A] (using decoder: Decoder[A]): Decoder[List[A]] = 
    Decoder.fromPartialFunction {
      case Json.Arr(list) => list.map(x => decoder.decode(x).get)
    }
  ...
  def field[A](name: String)(using decoder: Decoder[A]): Decoder[A] =
     Decoder.fromPartialFunction(f => f match 
        case Json.Obj(a) => decoder.decode(a.get(name).get).get
        )
...
trait ContactsCodecs:
  ...
  given Encoder[Contacts] = 
    ObjectEncoder.field[List[Person]]("people").transform[Contacts](contact => contact.people)
